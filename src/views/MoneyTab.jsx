...
// EMPTY_FORM moved to constants.js
          ...
          {Object.keys(CATEGORY_CONFIG).map(cat => {
            ...
            return (
              <button
                ...
                className={cn(
                  'flex flex-col items-center gap-1 py-3 rounded-2xl text-[10px] font-semibold transition-all active:scale-95',
                  form.category === cat
...
function InvestmentInput({ category, date, onSave, defaultValue = 0 }) {
  ...
  if (!editing && val === 0) {
    return (
      <button 
        ...
        className="flex items-center justify-between w-full p-3 rounded-2xl bg-surface-container hover:bg-surface-container-high transition-all"
      >
    ...
    return (
      <div className="flex items-center gap-2 w-full p-2 rounded-2xl bg-primary/5 border border-primary/20">
        <input 
          type="number"
          autoFocus
          className="bg-transparent w-full text-sm font-bold text-primary focus:outline-none px-2"
          value={val}
          onChange={e => setVal(e.target.value)}
          onBlur={save}
          onKeyDown={e => e.key === 'Enter' && save()}
        />
        <button onClick={save} className="text-primary p-1">
  ...
  return (
    <button 
      onClick={() => setEditing(true)}
      className="flex items-center justify-between w-full p-3 rounded-2xl bg-secondary/10 border border-secondary/20 transition-all"
    >
...
export default function MoneyTab() {
  ...
  const loadData = async () => {
    ...
    // Load Investments for the selected date
  ...
  return (
    <div className="flex flex-col min-h-screen">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            {selectedMonth !== getMonthStr() && (
              <button 
                onClick={() => setSelectedMonth(getMonthStr())}
      ...
      <div className="mx-4 mt-6">
        <div className="flex items-center justify-between mb-3 px-1">
          <span className="text-xs font-bold uppercase tracking-widest text-outline-variant">Breakdown</span>
        </div>
        <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-hide">
          {Object.keys(catTotals).length === 0 ? (
            ...
            Object.entries(catTotals).map(([cat, amt]) => {
              const cfg = CATEGORY_CONFIG[cat] || {};
              return (
                <div key={cat} className="card-floating p-3 flex-shrink-0 min-w-[100px]">
                  <p className="text-[10px] font-bold text-outline uppercase truncate mb-1">{cat}</p>
                  <p className="text-sm font-headline font-bold text-on-surface">₹{amt.toLocaleString()}</p>
                </div>
              );
            })
          )}
